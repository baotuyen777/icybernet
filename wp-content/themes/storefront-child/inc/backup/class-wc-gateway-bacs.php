...
class WC_Gateway_BACS extends WC_Payment_Gateway
{
    ...
    private function bank_details($order_id = '')
    {
        ...
        // Get sortcode label in the $locale array and use appropriate one.
        $sortcode = isset($locale[$country]['sortcode']['label']) ? $locale[$country]['sortcode']['label'] : __('Chi nhánh', 'woocommerce');
        ...
        if (!empty($bacs_accounts)) {
            ...
            foreach ($bacs_accounts as $bacs_account) {
                ...
                $bg = $bacs_account->iban ? 'style="background:' . $bacs_account->iban . '"' : '';
                $account_html .= "<ul class='wc-bacs-bank-details order_details bacs_details' $bg>" . PHP_EOL;
