...
define( 'NICKX_PLUGIN_VERSION', '1.3.9' );
...
function nickx_activation_hook_callback() {
	...
	if ( empty( get_option( 'nickx_slider_layout' ) ) ) {
		...
		update_option( 'nickx_place_of_the_video', 'yes' );
...
class WC_PRODUCT_VIDEO_GALLERY {
							...
							<table class="form-table">
								<tbody>
									...
									<tr valign="top" ' . ( ( $lic_chk_stateus ) ? '' : 'class="primium_aria" title="AVAILABLE IN PREMIUM VERSION"' ) . '">
										...
										<td class="forminp forminp-checkbox">
											<input name="nickx_show_only_video" id="nickx_show_only_video" type="checkbox" class="" value="yes" ' . checked( 'yes', get_option( 'nickx_show_only_video', 'no' ), false ) . '>
	...
	public function techno_wc_product_video_pro_html() {
		...
		echo '<div class="col-50">
			...
			<p class="paid_color">05. Adaptive Height (Slider Height Based On Images Automatically).</p>
	...
	public function nickx_meta_extend_call( $product_id ) {
		...
		$product_video_thumb_ids = get_post_meta( $product_id, '_product_video_thumb_url', true );
	...
	public function save_wc_video_url_field( $post_id ) {
		...
		if ( isset( $_POST['product_video_thumb_url'] ) ) {
			update_post_meta( $post_id, '_product_video_thumb_url', array_map( 'sanitize_text_field', $_POST['product_video_thumb_url'] ) );
		} else {
			delete_post_meta( $post_id, '_product_video_thumb_url' );
		}
...
function nickx_get_nickx_video_schema()
{
	if( is_product() ){
		...
		$product_video_types = get_post_meta( $product_id, '_nickx_product_video_type', true );
		$product_video_urls  = get_post_meta( $product_id, '_nickx_video_text_url', true ); 
		$video_thumb_ids     = get_post_meta( $product_id, '_product_video_thumb_url', true );
		$custom_thumbnails   = get_post_meta( $product_id, '_custom_thumbnail', true );
		$product_video_urls  = get_post_meta( $product_id, '_nickx_video_text_url', true );
		$video_schemas       = get_post_meta( $product_id, '_video_schema', true );
		$video_upload_dates  = get_post_meta( $product_id, '_nickx_video_upload_date', true );
			...
			foreach ($product_video_urls as $key => $product_video_url) {
				if( !empty( $product_video_url ) && isset($video_schemas[$key]) && $video_schemas[$key] == 'yes' && !empty( $video_names[$key] ) && !empty( $video_upload_dates[$key] ) && !empty( $video_descriptions[$key] ) ) {
					...
					if ( $product_video_type == 'nickx_video_url_youtube' ) {
						...
						$video_url_id = $matches[2];
						$embedurl = 'https://www.youtube.com/embed/' . $video_url_id . '?rel=0';
						$thumbnailurl = 'https://img.youtube.com/vi/' . $video_url_id . '/hqdefault.jpg';
					} else if ( $product_video_type == 'nickx_video_url_vimeo' ) {
						$embedurl = $product_video_url;
				        $data = json_decode( file_get_contents( 'http://vimeo.com/api/oembed.json?url=' . $embedurl ) );
					    if( $data ){
					    	$thumbnailurl = $data->thumbnail_url;
					    }
					} else if ( $product_video_type == 'nickx_video_url_local' ) {
						$product_video_thumb_url = wc_placeholder_img_src();
...
function nickx_get_nickx_video_html( $product_video_url, $extend, $key = 1 )
{
	if ( strpos( $product_video_url, 'youtube' ) > 0 || strpos( $product_video_url, 'youtu' ) > 0 ) {
		...
		return '<div class="tc_video_slide"><iframe style="display:none;" data-skip-lazy="" width="100%" height="100%" class="product_video_iframe" video-type="youtube" data_src="' . esc_url( $product_video_url ) . '" src="" frameborder="0" allow="autoplay; accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
	}
}
	...
	if ( has_post_thumbnail() || ! empty( $product_video_urls ) ) {
		...
		if( get_option( 'nickx_show_only_video' ) == 'yes' && $extend->is_nickx_act_lic() ){
			...
			if( !empty ( $product_image ) ){
				$html .= sprintf( '<div class="zoom">%s<span href="%s" class="nickx-popup fa fa-expand" data-fancybox="product-gallery"></span></div>', $product_image, $imgfull_src );
			}
		...
		if ( is_array($product_video_urls) ) {
			...
			foreach ($product_video_urls as $key => $product_video_url) {
				if( !empty( $product_video_url ) ) {
					...
					echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', '<li title="video" class="video-thumbnail"><div class="video_icon_img" style="background: url( ' . plugins_url( 'css/mejs-controls.svg', __FILE__ ) . ' ) no-repeat;"></div><img width="' . $gallery_thumbnail_size['width'] . '" height="' . $gallery_thumbnail_size['height'] . '" data-skip-lazy="" global-thumb="' . esc_url( get_option( 'custom_icon' ) ) . '" src="' . esc_url( $product_video_thumb_url ) . '" custom_thumbnail="' . esc_attr( $custom_thumbnail ) . '" class="product_video_img img_'.$key.' attachment-thumbnail size-thumbnail" alt="" sizes="(max-width: 150px) 100vw, 150px"></li>', '', $post->ID );
					if(!$extend->is_nickx_act_lic()){
						break;
					}
				}
			...
			echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', '<li title="video" class="video-thumbnail"><div class="video_icon_img" style="background: url( ' . plugins_url( 'css/mejs-controls.svg', __FILE__ ) . ' ) no-repeat;"></div><img width="' . $gallery_thumbnail_size['width'] . '" height="' . $gallery_thumbnail_size['height'] . '" data-skip-lazy="" global-thumb="' . esc_url( get_option( 'custom_icon' ) ) . '" src="' . esc_url( $product_video_thumb_urls ) . '" custom_thumbnail="' . esc_attr( $custom_thumbnails ) . '" class="product_video_img img_0 attachment-thumbnail size-thumbnail" alt="" sizes="(max-width: 150px) 100vw, 150px"></li>', '', $post->ID );
		}
	} else {
		return;
	}
